...
### Optional: 'voom' weights, transformation, and normalization

In many cases, power to detect differential exon usage can be further improved by using exon-level precision weights and normalizing across samples.

The `voom_weights()` function uses `limma-voom` to calculate exon-level precision weights, transform count data to log2-counts per million (i.e. on a continuous scale), and normalize across samples using scale normalization.
...
In this example, we will demonstrate how to use both the `voom` weights and the transformation/normalization. However, the default setting in the `regsplice()` wrapper function (which was shown above) is to use only the weights. The `regsplice` model fitting functions are designed to work with both counts and continuous data, so you can choose whether to use the original counts or the continuous transformed/normalized data.
...
# transformed/normalized data (skip this line to use weights only; see above)
Y <- out_voom$Y
```
