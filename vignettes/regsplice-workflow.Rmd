...
rs_results <- suppressWarnings(fit_reg_multiple(rs_results, rs_data, n_cores = 1, seed = seed))

# fit null models
rs_results <- fit_null_multiple(rs_results, rs_data, seed = seed)

# fit "full" models (not required if 'when_null_selected = "ones"' in next step)
rs_results <- fit_full_multiple(rs_results, rs_data, seed = seed)
```
...
```{r}
summary_table(rs_results, n = Inf)
```
