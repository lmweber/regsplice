...
After the results have been calculated (either with the wrapper function or the individual functions), `summary_table()` can be used to display the results in a more readable format.

The function `summary_table()` creates a data frame containing results for the the top `n` most highly significant genes, ranked according to either the false discovery rate (FDR) or raw p-values, up to a specified significance threshold.
...
Note that we have used `n = 10` for display purposes in the vignette. The p-values and adjusted p-values (FDRs) shown here are equal to zero due to the strong differential splicing signal for the simulated genes in this data set.
